...
/**
 * Class for meta data from the episode
 *
 */
class episode
{
     ...
     * @param array $item
     */
    public function __construct($main, $ITEMFILE, $feedattrs, $slug, $auphonic = false, array $item = array())
    {
        ...
        */
         ...
         */
        if ($item['chapters']) usort($item['chapters'], function ($a, $b) {
        ...
        // new: intro / 3 RC
        $itemIntro = explode(", ", $item['intro']);
        $item['intro_title'] = $itemIntro[0];
        $item['intro_owner'] = $itemIntro[1];
        $item['intro_ownerUrl'] = $itemIntro[2];
        $item['intro_lic'] = $itemIntro[3];
        $item['intro_licUrl'] = $itemIntro[4];

        $itemOutro = explode(", ", $item['outro']);
        $item['outro_title'] = $itemOutro[0];
        $item['outro_owner'] = $itemOutro[1];
        $item['outro_ownerUrl'] = $itemOutro[2];
        $item['outro_lic'] = $itemOutro[3];
        $item['outro_licUrl'] = $itemOutro[4];
     ...
     * @return array
     */
    public function parseConfig($main, $filename, $feedattrs, $item)
    {
        /*
         *  Parse an .epi file
         *
         * if item is given, it's a reparsing for overwriting
         * data from an auphonic-episode
         */
        $mime = $main->get('mimetypes');
        if (is_array($item) && count($item) == 0) {
            foreach ($main->get('itemattr') as $var) {
                $item = [$var];
            }
        }
        ...
        while (!feof($fh)) {
            ...
            if (substr($line, -1) == ":" && (in_array(substr($line, 0, -1), $main->get('itemattr')) || in_array(substr($line, 0, -1), $feedattrs['audioformats']))) {
                 ...
                 * A chapter line
                 *
                 * no link or image atm will have to separate
                 * with or because of title attribute,
                 * which might contain white space
                 */
                 ...
                 *
                 * only audioformats allowed,
                 * that are configured in feed.cfg
                 */
                $audio = explode(" ", $line);
    ...
    public function parseAuphonic($main, $filename, $feedattrs)
    {
        ...
        // images in chapters are located on auphonics server.
     ...
     * and kept for nostalgic reasons
     */
