...
$main->set('EXTDIR', './ext');
$main->set('TEMP', 'templates/tmp/');
$main->set('BITMASK', ENT_NOQUOTES | ENT_XML1);
$main->set('UI', '');

$main->set('TptPath', 'templates/');
$main->set('templatepath', 'templates/default/');
$main->set('favicon', 'templates/favicon.ico');
$main->set('favicon_apple', 'templates/favicon.png');
...
$main->set('firtzattr_default',
    array('feedalias', 'baseurlredirect')
);
$main->set('feedattr_default',
    array(
        'title',
        'description',
        'subtitle',         # 3.0 RC / Update podlove
        'summary',
        'formats',
        //impressum
        'author',
        ...
        'city',             # 3.0 RC
        'country',          # 3.0 RC
        'phone',            # 3.0 RC (optional)
        'fax',              # 3.0 RC (optional)
        'tax',              # 3.0 RC / Umsatzsteuer (optional)
        //poster
        'image',            # 3.0 RC
        'imageLicense',     # 3.0 RC
        'imageCreator',     # 3.0 RC
...
$main->set('extattr',
    array('slug', 'template', 'arguments', 'prio', 'script', 'type', 'vars', 'episode-vars', 'feed-vars')
);
$main->set('mimetypes',
    array(
        'mp3' => 'audio/mpeg',
...
$main->route('GET /search',
    function ($main) {
        if ( isset($_GET["s"]) && !empty($_GET["s"]) && isset($_GET["f"]) && !empty($_GET["f"]) ) {
            header('location: ' . @$_GET["f"] . '/show/search/' . @$_GET["s"] . '');
        }
    }, $main->get('CDURATION')
);
