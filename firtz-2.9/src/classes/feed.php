 ...
 * it does not represent a single feed to output
 * but all data needed to create a podcast with
 * multiple feeds
 **/
class feed
{
    ...
    public function __construct($main, $slug, $configfile)
    {
        ...
        // license intro (RC 3.0)
        $feedIntro = explode(", ", $attr['intro']);
        $attr['intro_title'] = $feedIntro[0];
        $attr['intro_owner'] = $feedIntro[1];
        $attr['intro_ownerUrl'] = $feedIntro[2];
        $attr['intro_lic'] = $feedIntro[3];
        $attr['intro_licUrl'] = $feedIntro[4];

        // license outro (RC 3.0)
        $feedOutro = explode(", ", $attr['outro']);
        $attr['outro_title'] = $feedOutro[0];
        $attr['outro_owner'] = $feedOutro[1];
        $attr['outro_ownerUrl'] = $feedOutro[2];
        $attr['outro_lic'] = $feedOutro[3];
        $attr['outro_licUrl'] = $feedOutro[4];

        // project banner
        $main->set('forgetByProject', $attr['supporter']);
